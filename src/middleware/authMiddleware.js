...
async function authMiddleware(req, res, next) {
    ...
    try {
        ...
        if (!accessToken) {
            return res.status(401).json({ message: 'Format token akses tidak benar' });
        }
        ...
        if (accessToken != tokenCookies) {
            return res.status(401).json({ message: 'Token cookies tidak valid' });
        }
        ...
        if (accessToken != user.token) {
            return res.status(403).json({ message: 'Token akses tidak berlaku' });
        }
        ...
        if (error.name === 'TokenExpiredError') {
            ...
            return res.status(401).json({ message: 'Token akses sudah kedaluwarsa' });
        } else if (error.name === 'JsonWebTokenError') {
            console.error('JsonWebTokenError:', error); // Logging error
            return res.status(401).json({ message: `Token akses tidak valid: ${error.message}` });
        } else {
